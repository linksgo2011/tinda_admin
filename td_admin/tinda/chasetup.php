...
<?php
	...
	if($_POST["tjbx-abc"]=='h-chi-vgs-8-com')
	{
		...
		$sql1="update hchi_pscxsz set pz_shul='$pz_shul',pz_off='$pz_off' where id='".$Aid."'";
		if(mysql_query($sql1))
		{
...
?>
      ...
      <ul class="forminfo">
    <li>
      <label>查询每天</label><input name="pz_shul" type="text" class="dfinput" id="pz_shul" value="<?php echo $rows1["pz_shul"]?>" style="width:80px;"/>&nbsp;次    </li>
    <li>
      <label>密码</label>
      <select name="pz_off" id="pz_off" class="dfinput" style="width:80px;">
        <option value="1" <?php if($rows1["pz_off"]==1){echo 'selected="selected"';}?>>开启</option>
        <option value="2" <?php if($rows1["pz_off"]==2){echo 'selected="selected"';}?>>关闭</option>
      </select>
    </li>
