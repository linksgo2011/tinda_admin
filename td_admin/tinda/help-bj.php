...
<?php
	$cid=$_GET["Aid"];
	$sql="select * from hchi_help where id=$cid";
	$rs=mysql_query($sql);
	...
	if($_POST["tjbx-abc"]=='h-chi-vgs-8-com')
	{
		...
		$sql1="update hchi_help set h_title='$h_title',h_body='$h_body' where id='".$Aid."'";
		if(mysql_query($sql1))
		{
		echo "<script language=javascript>alert('编辑成功！');window.location='help.php'</script>";
		}
		die();
	}
?>
<form id="form1" name="form1" action="help-bj.php" method="post">
      <input name="tjbx-abc" type="hidden" id="tjbx-abc" value="h-chi-vgs-8-com" />
      <input name="id" type="hidden" id="id" value="<?php echo $rows["id"]?>" />
    <ul class="forminfo">
    <li>
      <label>项目标题</label><input name="h_title" type="text" class="dfinput" id="h_title" value="<?php echo $rows["h_title"]?>" />
    </li>
    <li>
      <label>详&nbsp;&nbsp;情</label><textarea id="editor_id" name="content" style="width:90%;height:450px;"><?php echo htmlspecialchars($rows["h_body"])?></textarea>
    </li>
