...
<?php
	...
	$paix1=$_POST["paix1"];
      if($url1==""){
		$sql1="update finl set name='$name1',logoA='$logoA',paix1='$paix1' where id=$xg_id";
	  }else{
		$sql1="update finl set name='$name1',url1='$url1',logoA='$logoA',paix1='$paix1' where id=$xg_id";
	  }
		if (mysql_query($sql1))
		{
...
<form id="form1" name="form1" action="chaxfinl.php" method="post">
      <input name="tjbx-abc1" type="hidden" id="tjbx-abc2" value="h-chi-vgs-8-com" />
    <ul class="forminfo">
    <li>
      <label>分类</label>
...
<form name="form1" method="post" action="">
    ...
    <li>||----品牌:
      ...
      &nbsp;&nbsp;<a href="?Aid=<?php echo $rows1["id"]?>&edit=del" onClick="return confirm('确定删除吗?将不可恢复！');" class="tablelink" style="padding:5px 12px 5px 12px;background-color:#ff0000;color:#FFF;"> 删除</a>    </li>
    </ul>
    </form> 
<!--------------------------------------------->
<?php
...
<form name="form1" method="post" action="">
    ...
    <li>|||----:
      ...
      &nbsp;&nbsp;<input type="submit" name="xiugan" id="xiugan" value="确认" class="btn1">&nbsp;&nbsp;<a href="chax-body.php?Aid=<?php echo $rows2["id"]?>">录入祥情</a>&nbsp;&nbsp;<a href="?Aid=<?php echo $rows2["id"]?>&edit=del">删除</a>
      </li>
    </ul>
    </form> 
<?php }}}?>      
